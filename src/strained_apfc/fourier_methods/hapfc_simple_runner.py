...
class FFTHydroSimpleAPFCSim:
    ...
    def build(self, con_sim: bool, config: dict):
        ...
        self.vel_lin_part_n0_variation = self.get_velocity_lin_n0_variation_hat()
    ...
    def get_velocity_lin_n0_variation_hat(self) -> np.ndarray:

        ret = np.array([complex(0, 1) * self.kxm, complex(0, 1) * self.kym])
    ...
    def calc_f_eta_variation(self, eta_i: int) -> np.ndarray:
        ...
        variation = self.etas_lin_part[eta_i] * self.etas_hat[eta_i]
        variation = np.fft.ifft2(variation, s=self.etas[0].shape)
        variation += self.eta_non_lin_term[eta_i]
    ...
    def calc_f_n0_variation(self) -> np.ndarray:
        ...
        non_lin_hat = np.fft.fft2(self.n0_non_lin_term)
        variation = self.n0_lin_part * self.n0_hat + non_lin_hat
    ...
    def calc_f_n0_non_lin_part(self) -> np.ndarray:

        dx = gradient_periodic_BC(self.n0_non_lin_term, self.dx, 0)
        dy = gradient_periodic_BC(self.n0_non_lin_term, self.dx, 1)
    ...
    def calc_velocity_non_lin_part(self) -> np.ndarray:
        ...
        f = self.calc_f_eta_part() / self.init_n0
        f += self.calc_f_n0_non_lin_part()

        ret = f - self.calc_advection_spatial_gradient(self.velocity)
    ...
    def velocity_routine(self) -> tuple[np.ndarray, np.ndarray]:
        ...
        n0_lin = -self.vel_lin_part_n0_variation * self.n0_lin_part * self.n0_hat

        n = self.calc_velocity_non_lin_part()
        n = self.vec2d_component_wise_fft(n)

        denom = 1.0 - self.velocity_dt * lagr
        n_v = self.velocity_hat + self.velocity_dt * (n + n0_lin)
        n_v = n_v / denom
